...
export default function ERDiagramPage() {
  ...
  useEffect(() => {
    ...
    // Ensure we don't initialize multiple diagrams on the same div
    const existingDiagram = go.Diagram.fromDiv(divRef.current);
    if (existingDiagram) {
    ...
    try {
      ...
      diagram.nodeTemplate = $(
        go.Node, "Auto",
        {
          ...
          toolTip: $(
            go.Adornment, "Auto",
            $(go.Shape, "RoundedRectangle", { fill: "#1a1d27", stroke: "#818cf8", strokeWidth: 1.5, parameter1: 6 }),
            $(go.Panel, "Vertical", { margin: 12, maxSize: new go.Size(280, NaN) },
              $(go.TextBlock, {
              ...
              }, new go.Binding("text", "schema", (s) => `Schema: ${s}`)),
              $(go.TextBlock, {
        ...
        $(go.Panel, "Vertical", { defaultAlignment: go.Spot.Left },
          ...
          $(go.Panel, "Auto", { stretch: go.GraphObject.Horizontal },
            $(go.Shape, "RoundedRectangle", { fill: "#1e2235", stroke: "transparent", parameter1: 8 }),
      ...
      if (e instanceof Error) {
        ...
        setErrorObj(new Error(String(e)));
      }
    }
  }, []);
  ...
  if (errorObj) {
    return (
      <Box sx={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", p: 4 }}>
        <PageHeader title="ER Diagram Crash" subtitle="Failed to render diagram" />
        <Box sx={{ color: "#ff6b6b", p: 2, bgcolor: "#2d1b1b", mt: 2, borderRadius: 2 }}>
          <Box sx={{ fontWeight: "bold", mb: 1 }}>{errorObj.name}: {errorObj.message}</Box>
          <pre style={{ whiteSpace: "pre-wrap", fontSize: 12, opacity: 0.8 }}>{errorObj.stack}</pre>
        </Box>
      </Box>
    );
  }
